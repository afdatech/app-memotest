...
export default Component.extend({
    ...
    if (this.get("isLost") | this.get("isWin")) {
      if (this.get("isWin")) {
        this.set(
          "pointsBonus",
          (this.get("reminder") *
            this.get("currentLevel").get("comboModifier")) /
            10
        );
        this.set(
          "totalPoints",
          this.get("totalPoints") + this.get("pointsBonus")
        );
      }
      this.stopTimers();
      return true;
  ...
  cards: Ember.computed("currentLevel", function() {
    ...
    if (currentLevel) return currentLevel.get("cards");
    else return [];
  }),
  ...
  percentTimer: Ember.computed("duration", function() {
    return Math.floor(
      (this.get("duration") / this.get("currentLevel").get("time")) * 100
    );
  }),
